...
sealed trait Sink[-T] extends Any {
  ...
  /**
    ...
    */
  def <--[F[_] : Effect](observable: Observable[T]): F[Cancelable] = Effect[F].delay {
...
object Sink {
  ...
  /**
    ...
    */
  def create[T,F[_]](next: T => F[Future[Ack]],
                               error: Throwable => F[Unit]/* = (_:Throwable) => Effect[F].pure(Unit)*/,
                               complete: () => F[Unit]/* = () => Effect[F].pure(Unit)*/
                              )(implicit s: Scheduler,  e:Effect[F]): Sink[T] = {
    val sink = ObserverSink(
      new Observer[T] {
        override def onNext(t: T): Future[Ack] = {
          val p:Promise[Ack] = Promise[Ack]()
          next(t).runAsync {
            case Right(f) => IO { p.completeWith(f) }
            case Left(e) => IO { p.failure(e) }
          }
          p.future
        }

        override def onError(ex: Throwable): Unit = error(ex).runAsync(_ => IO.pure(Unit))

        override def onComplete(): Unit = complete().runAsync(_ => IO.pure(Unit))
      }
    )
    sink
  }
