...
//   }
  ...
  it should "correctly merge keys" in {
    ...
    val node =
      SnabbdomOps.toSnabbdom(input(attributes.key := "bumm")(attributes.key := "klapp"), RenderConfig.ignoreError)
  ...
  "svg" should "should work with tags and attributes" in {
    ...
    node.children.get.head.data.get.attrs.get.toMap shouldBe Map(
      "fill" -> "red",
