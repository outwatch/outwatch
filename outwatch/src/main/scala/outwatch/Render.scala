...
object Render {
  ...
  @noinline private def sourceToModifierRender[F[_] : Source, T: Render](source: F[T]): VDomModifier = StreamModifier(sink => Source[F].subscribe(source)(Observer.lift(sink).contramap(VDomModifier(_))))
