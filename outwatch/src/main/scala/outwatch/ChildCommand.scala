...
object ChildCommand {
  ...
  def stream(valueStream: Observable[Seq[ChildCommand]], config: RenderConfig): VMod = VMod.delay {
    ...
    val idToIndex: ChildId => Int = {
      case ChildId.Key(key) =>
        children.indexWhere { tree =>
          tree.proxy.key.fold(false)((k: Key.Value) => k == key)
        }
      case ChildId.Element(element) =>
        children.indexWhere { tree =>
