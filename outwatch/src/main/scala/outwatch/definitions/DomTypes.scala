...
private[outwatch] object BuilderTypes {
  type ReflectedAttribute[T, _]     = AttrBuilder[T, VMod]
  type Attribute[T]                 = AttrBuilder[T, VMod]
  type Property[T, _]               = AttrBuilder[T, VMod]
...
trait DocumentEvents
  ...
  def isKeyDown(keyCode: Int): Observable[Boolean] = Observable
    .merge(
...
private[outwatch] trait SimpleStyleBuilder extends builders.StyleBuilders[Style] {
  override protected def buildDoubleStyleSetter(style: keys.Style[Double], value: Double): Style =
    BasicStyle(style.name, value.toString)
  override protected def buildIntStyleSetter(style: keys.Style[Int], value: Int): Style =
    BasicStyle(style.name, value.toString)
  override protected def buildStringStyleSetter(style: keys.Style[_], value: String): Style =
    BasicStyle(style.name, value.toString)
}
