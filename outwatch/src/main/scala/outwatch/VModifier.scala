...
import outwatch.helpers.{BasicAttrBuilder, BasicStyleBuilder, PropBuilder, VModifierBooleanOps}
...
object VModifierM {
  ...
  @inline def managedSubscribe[F[_]: Source, T](source: F[T]): VModifier = managedDelay(
  ...
  @inline def managedDelay[T: CanCancel](subscription: => T): VModifier =
  ...
  object managedElement {
    ...
    @inline def asHtml[T: CanCancel](subscription: dom.html.Element => T): VModifier =
      apply(elem => subscription(elem.asInstanceOf[dom.html.Element]))

    @inline def asSvg[T: CanCancel](subscription: dom.svg.Element => T): VModifier =
  ...
  @inline def apply[Env](
    modifier: VModifierM[Env],
    modifier2: VModifierM[Env],
    modifier3: VModifierM[Env],
  ): VModifierM[Env] =
  ...
  @inline def apply[Env](
    ...
    modifier5: VModifierM[Env],
    modifier6: VModifierM[Env],
    modifier7: VModifierM[Env],
    modifiers: VModifierM[Env]*,
  ): VModifierM[Env] =
  ...
  @inline def composite[Env](modifiers: Iterable[VModifierM[Env]]): VModifierM[Env] =
    CompositeModifier[Env](modifiers.toJSArray)

  @inline def fromEither[Env, T: Render[Env, *]](modifier: Either[Throwable, T]): VModifierM[Env] =
    modifier.fold(raiseError(_), apply(_))
  @inline def delayEither[Env, T: Render[Env, *]](modifier: => Either[Throwable, T]): VModifierM[Env] =
  ...
  @inline class PartiallyAppliedAccessM[Env] {
    @inline def apply[R](modifier: Env => VModifierM[R]): VModifierM[Env with R] =
  ...
  @inline class VModifierSubscriptionOwner[Env] extends SubscriptionOwner[VModifierM[Env]] {
    @inline def own(owner: VModifierM[Env])(subscription: () => Cancelable): VModifierM[Env] =
      VModifierM(owner, VModifier.managedDelay(subscription()))
  }

  @inline implicit def renderToModifier[Env, T: Render[Env, *]](value: T): VModifierM[Env] =
...
final case class StreamModifier[-Env](subscription: Observer[VModifierM[Env]] => Cancelable)
...
object VNodeM {
  @inline final def html(name: String): HtmlVNode =
    BasicNamespaceVNodeM(name, js.Array[VModifier](), VNodeNamespace.Html)
  ...
  @inline class VNodeSubscriptionOwner[Env] extends SubscriptionOwner[VNodeM[Env]] {
    @inline def own(owner: VNodeM[Env])(subscription: () => Cancelable): VNodeM[Env] =
      owner.append(VModifier.managedDelay(subscription()))
  }
}
...
@inline final case class AccessEnvVNodeM[-Env](node: Env => VNode) extends VNodeM[Env] {
  ...
  def apply[R](args: VModifierM[R]*): AccessEnvVNodeM[Env with R] =
    copy(node = env => node(env).apply(VModifierM.composite(args).provide(env)))
  def append[R](args: VModifierM[R]*): AccessEnvVNodeM[Env with R] =
    copy(node = env => node(env).append(VModifierM.composite(args).provide(env)))
  def prepend[R](args: VModifierM[R]*): AccessEnvVNodeM[Env with R] =
    copy(node = env => node(env).prepend(VModifierM.composite(args).provide(env)))
}
...
@inline final case class BasicNamespaceVNodeM[+N <: VNodeNamespace, -Env](
  nodeType: String,
  modifiers: js.Array[_ <: VModifierM[Env]],
  namespace: N,
) extends VNodeM[Env] {
  ...
  def append[R](args: VModifierM[R]*): BasicNamespaceVNodeM[N, Env with R] =
    copy(modifiers = appendSeq(modifiers, args))
  def prepend[R](args: VModifierM[R]*): BasicNamespaceVNodeM[N, Env with R] =
    copy(modifiers = prependSeq(modifiers, args))
  def thunk[R](key: Key.Value)(arguments: Any*)(renderFn: => VModifierM[R]): ThunkVNodeM[Env with R] =
    ThunkVNodeM[Env with R](this, key, VNodeThunkCondition.Compare(arguments.toJSArray), () => renderFn)
  def thunkConditional[R](key: Key.Value)(shouldRender: Boolean)(renderFn: => VModifierM[R]): ThunkVNodeM[Env with R] =
    ThunkVNodeM[Env with R](this, key, VNodeThunkCondition.Check(shouldRender), () => renderFn)
  @inline def thunkStatic[R](key: Key.Value)(renderFn: => VModifierM[R]): ThunkVNodeM[Env with R] =
