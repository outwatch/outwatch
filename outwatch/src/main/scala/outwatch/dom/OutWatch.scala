...
object OutWatch {
  ...
  def toSnabbdom[F[_]: Sync](vNode: VNode)(implicit s: Scheduler): F[VNodeProxy] = Sync[F].delay {
    SnabbdomOps.toSnabbdom(vNode)
  }

  def renderInto[F[_]: Sync](element: dom.Element, vNode: VNode)(implicit s: Scheduler): F[Unit] =
  ...
  def render[F[_]: Sync](querySelector: String, vNode: VNode)(implicit s: Scheduler): F[Unit] = 
